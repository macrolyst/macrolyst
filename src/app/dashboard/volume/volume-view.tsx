...
export function VolumeView() {
  ...
  const [sortBy, setSortBy] = useState<"volume" | "volumeRatio" | "changePercent">("volume");
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
        ...
        <div className="flex gap-1 bg-(--surface-2) rounded-lg p-1">
          {([
            ...
            { key: "volumeRatio" as const, label: "Vol Ratio" },
            ...
            <button
              ...
              className={`px-3 py-1 rounded-md text-[11px] font-medium cursor-pointer transition-colors ${
                sortBy === s.key ? "bg-(--surface-3) text-white" : "text-(--text-secondary)"
              }`}
            >
              {s.label}
            </button>
          ...
          <div key={stock.symbol}>
            ...
            <div className="hidden sm:grid grid-cols-12 gap-2 px-4 py-2.5 border-b border-(--border) last:border-0 hover:bg-white/[0.02] transition-colors items-center">
              ...
              <div className="col-span-2 text-right">
                ...
                <p className="text-[10px] text-(--text-secondary)">avg {formatVolume(stock.avgVolume)}</p>
              </div>
